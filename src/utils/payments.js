...
const payments = {
  ...
  init: async () => {
    ...
    if (payments.isInitializing || payments.isLoaded) { return }

    try {
      payments.isInitializing = true
      ...
      payments.callEventListeners(EVENTS.LOADED)
    } catch (e) {
      console.warn('payments init e', e)
    } finally {
      payments.isInitializing = false
    }

    return payments.isLoaded
  },
  ...
  addEventListener: callback => {
    ...
    payments.eventListeners[callbackId] = callback
    return () => {
      delete payments.eventListeners[callbackId]
    }
  },
  callEventListeners: eventName => {
    for (const callback of Object.values(payments.eventListeners)) { callback(eventName) }
  }
}
...
// const removeEventListeners = payments.addEventListener(data => {
// })
